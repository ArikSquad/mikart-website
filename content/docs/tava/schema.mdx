...
<Callout type="warn" title="BigDecimal needs a shape">
    `BigDecimal` fields must declare `@Field(precision = ..., scale = ...)` so adapters can create deterministic native
    columns.
</Callout>
...
<Callout type="idea" title="Portable first, native when needed">
    Use logical types for the model everyone shares. Add adapter settings only for the places where the backend storage
    choice matters.
</Callout>
